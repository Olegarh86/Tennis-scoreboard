...
- Классы сервисов используют `sessionFactory.openSession()` (через `HibernateUtil.getSession()`) для получения сессии. Такое ручное управление сессиями имеет несколько недостатков:
...
Один из вариантов исправления — перейти на паттерн "Session-per-Request" ("сессия на запрос"). Его суть в том, что одна сессия Hibernate используется всеми сервисами и DAO на протяжении всей обработки HTTP-запроса. А также провести рефакторинг.
